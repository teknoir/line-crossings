// Utility functions to enrich an alert with line-crossings collection data
...
async function findLineCrossingDoc(db, parsed, alert, attempts) {
  ...
  attempts && attempts.push({ strategy: 'id_only_probe', query: { 'data.id': detectionId }, found: !!idOnlyProbe, annotationKeys: Object.keys(annotationValues), annotationValues });
  ...
  for (const directionKey of directionKeyCandidates) {
    ...
    for (const lineIdKey of lineIdKeyCandidates) {
      ...
      doc = await collection.find(query3)
        ...
        .limit(1)
        .next();
      attempts && attempts.push({ strategy: `lineid+peripheral(${directionKey},${lineIdKey})`, query: query3, found: !!doc });
      if (doc) return doc;
    }
    ...
    if (peripheralId) {
      ...
      let recentDocs = await collection.find(query4a)
        ...
        .limit(10)
        .toArray();
      attempts && attempts.push({ strategy: `recent_by_peripheral_id(${directionKey})`, query: query4a, candidates: recentDocs.length });
      ...
      if (recentDocs.length === 0) {
        ...
        recentDocs = await collection.find(query4b)
          ...
          .limit(10)
          .toArray();
        attempts && attempts.push({ strategy: `recent_by_peripheral_name(${directionKey})`, query: query4b, candidates: recentDocs.length });
      }
      ...
      if (recentDocs.length > 0 && alertStartTime) {
        ...
        attempts && attempts.push({ strategy: `time_proximity(${directionKey})`, picked: docPicked?.data?.id || null });
...
async function enrichAlert(alert, db, options = {}) {
  ...
  attempts && attempts.push({ strategy: 'primary_selection', parsedDirection: targetDir, primaryDirection: (primaryDoc.metadata?.annotations?.['teknoir.org/linedir'] || '').toLowerCase() });
